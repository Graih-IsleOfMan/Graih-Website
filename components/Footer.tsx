...
const social = [
    {
      name: "Facebook",
      href: "http://facebook.com/graihhomelesscharity",
      icon: (props: any) => (
    ...
    {
      name: "Twitter",
      href: "https://twitter.com/graih_iom",
      icon: (props: any) => (
        <svg fill="currentColor" viewBox="0 0 24 24" {...props}>
...
const Footer = ({ footerMenu, trustees }: Props) => {
  return (
    <footer className="bg-slate-800">
      <div className="max-w-7xl mx-auto py-12 px-4 overflow-hidden sm:px-6 lg:px-8">
        ...
        >
          {footerMenu.map((item) => (
            <div key={item.label} className="px-5 py-2">
              <span className="text-base text-slate-300 hover:text-white">
                <Link
                  href={item.href}
                >
                  {item.label}
        ...
        <p className="mt-8 text-center text-base text-slate-300">
          ...
          <br />
          Trustees: {trustees.map(t => t.name).join(', ')}
        </p>
      </div>
    </footer>
  );
